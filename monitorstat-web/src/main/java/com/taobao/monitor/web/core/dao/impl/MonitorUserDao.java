...
import com.taobao.monitor.web.vo.AlarmUserPo;
...
public class MonitorUserDao extends MysqlRouteBase{
	...
	public List<ExtraUserAppKeyDefine> getExtraUserAppKeyDefine(int userId){
		...
		final List<ExtraUserAppKeyDefine> list = new ArrayList<ExtraUserAppKeyDefine>();
		
		String sql = "select * from ms_monitor_extra_user_key_define where user_id = ?";
		
		
		try {
			this.query(sql, new Object[]{userId}, new SqlCallBack(){
				@Override
				public void readerRows(ResultSet rs) throws Exception {
					
					ExtraUserAppKeyDefine define = new ExtraUserAppKeyDefine();
					define.setAppId(rs.getInt("app_id"));
					define.setKeyId(rs.getInt("key_id"));
					define.setUserId(rs.getInt("user_id"));
					list.add(define);
				}});
		} catch (Exception e) {
		}
