	...
	public ArkAuthenRequest(HttpServletRequest request, HttpServletResponse response) throws JSONException, RedirectException {
		...
		if (cookies != null && cookies.length > 0) {
			for (Cookie cookie : cookies) {
				if (StringUtil.equals(COOKIENAME, cookie.getName())) {
					httpcookie = cookie;
					break;
				}
			}
		...
		if (httpcookie != null && StringUtil.isNotBlank(httpcookie.getValue())) {
			...
			if (StringUtil.isNotBlank(cookiecode)) {
				try {
				...
				//解密出来的值不是特定的状态代码
				//强制将其修正为默认状态代码
				if (!StringUtil.equals(String.valueOf(CodeDefine.DEFAULT), cookiecode)
						&& !StringUtil.equals(String.valueOf(CodeDefine.MIDDLE), cookiecode)
						&& !StringUtil.equals(String.valueOf(CodeDefine.FINAL), cookiecode)) {
			...
			if (!secretkeyiv.isIsexpired()) {
				...
				case CodeDefine.DEFAULT:
					...
					if (secretkeyiv.getSecretexp().compareTo(convertTz()) <= 0) {
						...
						if (StringUtil.isNotBlank(decryptjson)) {
