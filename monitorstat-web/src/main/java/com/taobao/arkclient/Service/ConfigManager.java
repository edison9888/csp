	...
	public ConfigManager() {
		...
		try {
			if (propFile.exists()) {
				...
				String webInfoPath = StringUtil.substringBeforeLast(classPathUrl, "classes/");
		...
		if (null != p) {
			if (!p.isEmpty()) {
				...
				for (Object keyObj : p.keySet()) {
					...
					if (StringUtil.isNotBlank(key) && key.startsWith("unProjectURL")) {
						ConfigManager.unProtectList.add(StringUtil.defaultIfBlank(p.getProperty(key)));
					}
				}
			}
			//设置默认参数
