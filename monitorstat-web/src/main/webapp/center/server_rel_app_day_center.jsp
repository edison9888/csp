...
<a id="addIframe" href="./server_rel_app_day_add.jsp?serverId=<%=id %>" STYLE="text-decoration:underline"><font color='red'>-->添加应用关联</font></a> &nbsp; 
</div>
<div id="dialog" class="ui-dialog-content ui-widget-content">
<%
List<AppInfoPo> appList = AppInfoAo.get().findAllAppByServerIdAndAppType(Integer.parseInt(id),"day");
...
<table width="100%" border="1" class="ui-widget ui-widget-content" id="userInfoTableId">
	<tr>
		...
		<td align="center">OpsName</td>
		<td align="center">机器数量</td>
	</tr>
	<%for(AppInfoPo po: appList){	
		List<HostPo> list = CspCacheTBHostInfos.get().getHostInfoListByOpsName(po.getOpsName());
	 %>
	<tr >
		<td align="center"><%=po.getAppId()%></td>
		<td align="left"><a target="_blank" href="http://cm.taobao.net:9999/monitorstat/app_detail.jsp?selectAppId=<%=po.getAppDayId() %>"><%=po.getAppName()%></a></td>
		<td align="left"><%=po.getOpsName()%></td>
