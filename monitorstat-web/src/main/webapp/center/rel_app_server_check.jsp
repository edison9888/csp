...
function goToAdd(){
	...
	var url = "<%=request.getContextPath () %>/center/rel_app_server_add.jsp?&appId=<%=appId%>&appName=<%=appName%>";
	location.href=url;
}
...
List<AppInfoPo> appInfoPoList = AppInfoAo.get().findServerRel(Integer.parseInt(appId));
...
<table align="center" width="400" border="1" class="ui-widget ui-widget-content">
	...
	<tr>
		...
		<td>
			<a href="./rel_app_server_update.jsp?appId=<%=po.getAppId() %>&appName=<%=po.getAppName() %>&serverId=<%=po.getServerInfoPo().getServerId() %>&appType=<%=po.getAppType()%>">修改</a>&nbsp;&nbsp;
			<a href="./rel_app_server_check.jsp?appId=<%=po.getAppId() %>&serverId=<%=po.getServerInfoPo().getServerId()%>&appName=<%=po.getAppName() %>&appType=<%=po.getAppType() %>&action=delete">删除</a> &nbsp;&nbsp;
		</td>
	</tr>
...
<table align="center" width="400" border="1" class="ui-widget ui-widget-content">
	...
	<tr>
	<td align="center" >
		<a href="./rel_app_server_add.jsp?appId=<%=appId %>&appName=<%=appName %>"><font size="5">还没有关联的服务器，请添加关联</font></a>
	</td>
	</tr>
		<%
	}
