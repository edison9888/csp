...
function goToCheck(){
	...
	var url = "<%=request.getContextPath () %>/center/db_rel_app_time_center.jsp?databaseId=<%=databaseId%>";
	location.href=url;
}
...
%>
<form action="./db_rel_app_time_add.jsp" onsubmit="return checkAlarm()"  method="post" name="myform" id="myform">
<%
...
if("add".equals(action)){
	...
	%>
		<font size="+3" color="red"><%if(b){out.print("添加成功	<input type='button' value='关闭' onclick='parent.location.reload()'>");}else{out.print("失败!出现异常");} %></font>	
			<!--<a href="./db_rel_app_time_center.jsp?serverId=<%=databaseId%>">关闭</a>	-->
			<%
}
	...
	List<AppInfoPo>  appList = AppInfoAo.get().findAllAppWithoutDatabaseRel("time");
