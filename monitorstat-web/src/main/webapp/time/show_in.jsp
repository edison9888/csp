...
<script type="text/javascript">
	...
	function openTime(keyId){
		...
		window.open("./key_detail_time.jsp?keyId="+keyId+"&appName=" + "<%=appInfopo.getAppName()%>");
...
<%	
{
Map<String,List<KeyValuePo>> outHsfKeyMap = MonitorTimeAo.get().findLikeKeyAverageByLimit5("IN_HSF-ProviderDetail",appInfopo.getAppName());
...
<table id="IN_HSF-ProviderDetail" width="100%" border="1" class="ui-widget ui-widget-content">
	...
	for(Map.Entry<String,List<KeyValuePo>> entry:outHsfKeyMap.entrySet()){		
		...
		if(keys[1].equals("HSF-ProviderDetail-BizException")){
			...
			if(keyName.indexOf(Constants.COUNT_TIMES_FLAG)>-1){				
				...
				hsf.setExeCountBaseLine(baseLineValue);
			}
			if(keyName.indexOf(Constants.AVERAGE_USERTIMES_FLAG)>-1){
				...
				hsf.setAverageExeBaseLine(baseLineValue);
			}										
	...
	while(sortOutMapit.hasNext()){
		...
		for(HsfClass hc:hsfClassList){
			...
			%>					
			<tr>
				<td align="center" width="200"><%=classShow?po.getClassName():" - " %></td>
				<td align="center" width="200"><%=po.getMethodName() %></td>
				<td align="center" width="60"><%=po.getCollectTime() %></td>
				<td align="center" width="60"><img src="<%=request.getContextPath () %>/statics/images/report.png" onclick="openTime(<%=po.getCountkeyId() %>)"/><%=Utlitites.fromatLong(po.getExeCount()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=po.getCountkeyId() %>&appName=<%=appInfopo.getAppName() %>' title='将此key加入告警' target="_blank">加</a></td>					
				<td align="center" width="60"><%=Utlitites.fromatLong(po.getExeCountBaseLine()) %>&nbsp&nbsp<%=Utlitites.scale(String.valueOf(po.getExeCountNum()), po.getExeCountBaseLine()) %></td>
				<td align="center" width="60"><img src="<%=request.getContextPath () %>/statics/images/report.png" onclick="openTime(<%=po.getAverageKeyId() %>)"/><%=Utlitites.formatDotTwo(po.getAverageExe()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=po.getAverageKeyId() %>&appName=<%=appInfopo.getAppName() %>' title='将此key加入告警' target="_blank">加</a></td>				
				<td align="center" width="60"><%=Utlitites.fromatLong(po.getAverageExeBaseLine()) %>&nbsp&nbsp<%=Utlitites.scale(po.getAverageExe(), po.getAverageExeBaseLine()) %></td>			
				<td align="center" width="60"><%=po.getBizExecptionNum()==null?" - ":po.getBizExecptionNum() %>/<%=po.getExecptionNum()==null?" - ":po.getExecptionNum() %></td>					
			</tr>					
			<%
			aimShow = false;
			classShow = false;	
