...
<script type="text/javascript">
	...
	function openTime(keyId){
		...
		if(parseInt(keyId) != keyId) {
			...
			window.open("./key_detail_time.jsp?keyId="+keyId+"&appName=" + "<%=appName%>" + "&collectTime1=" + "<%=collectTime1%>");
		}
	}
...
for(int i=0;i<1;i++){
	...
	for(Map.Entry<String,KeyValuePo> entry:searchVo.getMap().entrySet()){		
		...
		// 获取当前key当前时间的基线数据
		KeyValueBaseLinePo baseLinePo = MonitorBaseLineAo.get().findKeyBaseValueByDate(appInfopo.getAppId(), po.getKeyId(), searchVo.getDate());
		String baseLineValue;
		if(baseLinePo != null) {
			baseLineValue = String.valueOf(baseLinePo.getBaseLineValue());
		} else {
			baseLineValue = "0";
		}
		...
		if(key.indexOf(Constants.COUNT_TIMES_FLAG)>-1){
			...
			searchEnginePo.setExeCountBaseLine(baseLineValue);
		}
		if(key.indexOf(Constants.AVERAGE_USERTIMES_FLAG)>-1){
			...
			searchEnginePo.setExeAverageBaseLine(baseLineValue);
		}
	}
...
<table id="OUT_SearchEngine" width="100%" border="1" class="ui-widget ui-widget-content">
	...
	<tr class="ui-widget-header ">
	  ...
	  <td align="center" width="40"><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=entry.getValue().getCountkeyId() %>)"/><%=entry.getValue().getExeCount() %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=entry.getValue().getCountkeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
	  <!-- <td align="center" width="40"><%=po1==null?0:po1.getValueStr() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getExeCount(),po1==null?null:po1.getValueStr()) %></td>	 --> 
	  <td align="center" width="40"><%=entry.getValue().getExeCountBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(Utlitites.formatDotTwo(entry.getValue().getExeCount()), Utlitites.formatDotTwo(entry.getValue().getExeCountBaseLine())) %></td>
	  <td align="center" width="40"><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=entry.getValue().getAverageKeyId() %>)"/><%=Utlitites.formatDotTwo(entry.getValue().getExeAverage()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=entry.getValue().getAverageKeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
	  <!-- <td align="center" width="40"><%= po2==null?0:po2.getValueStr() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getExeAverage(),po2==null?null:po2.getValueStr()) %></td>	 --> 
	  <td align="center" width="40"><%=Utlitites.formatDotTwo(entry.getValue().getExeAverageBaseLine()) %>&nbsp&nbsp<%=Utlitites.scale(Utlitites.formatDotTwo(entry.getValue().getExeAverage()), Utlitites.formatDotTwo(entry.getValue().getExeAverageBaseLine())) %></td>
	</tr>
  	<%} %>			
</table>
<%} }%>
...
for(int i=0;i<1;i++){
	...
	for(Map.Entry<String,KeyValuePo> entry:searchVo.getMap().entrySet()){		
		...
		if(key.indexOf(Constants.COUNT_TIMES_FLAG)>-1){
			...
			tairClientPo.setExeCountBaseLine(baseLineValue);
		}
		if(key.indexOf(Constants.AVERAGE_USERTIMES_FLAG)>-1){
			tairClientPo.setAverageExe(po.getValueStr());
			tairClientPo.setAverageKeyId(po.getKeyId());
			tairClientPo.setAverageExeBaseLine(baseLineValue);
		}
	}
...
<table id="OUT_TairClient" width="100%" border="1" class="ui-widget ui-widget-content">
	...
	<tr class="ui-widget-header ">
	  ...
	  <td align="center" width="40"><%=entry.getValue().getExeCountBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getExeCount(), entry.getValue().getExeCountBaseLine()) %></td>
	  <td align="center" width="40"><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=entry.getValue().getAverageKeyId() %>)"/><%=Utlitites.formatDotTwo(entry.getValue().getAverageExe()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=entry.getValue().getAverageKeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
	  <!-- <td align="center" width="60"><%=Utlitites.scale(entry.getValue().getAverageExe(),po2==null?null:po2.getValueStr()) %></td> -->
  	  <td align="center" width="40"><%=entry.getValue().getAverageExeBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getAverageExe(), entry.getValue().getAverageExeBaseLine()) %></td>
  	</tr>
  	<%} %>			
</table>
<%} }%>
...
<table id="OUT_forest" width="100%" border="1" class="ui-widget ui-widget-content">
	...
	<tr class="ui-widget-header ">
	  ...
	  <td align="center" width="60"><%=entry.getValue().getExeCountBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getExeCount(),entry.getValue().getExeCountBaseLine()) %></td>
	  ...
	  <td align="center" width="60"><%=entry.getValue().getAverageExeBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getAverageExe(),entry.getValue().getAverageExeBaseLine()) %></td>	 
  	</tr>
  	<%} %>			
</table>
<%} }%>
...
<table id="OUT_PageCache" width="100%" border="1" class="ui-widget ui-widget-content">
	...
	<tr class="ui-widget-header ">
  	...
  	%>
  	<tr>  	   	
	  <td align="center" width="40"><%=_keys[0] %></td>
	  <td align="center"><%=_keys[1] %></td>
	  <td align="center"><%=_keys[2] %></td>
	  <td align="center"><%=time %></td>	  
	  <td align="center" width="60"><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=entry.getValue().getCountkeyId() %>)"/><%=entry.getValue().getExeCount() %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=entry.getValue().getCountkeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
	  <td align="center" width="60"><%=entry.getValue().getExeCountBaseLine() %>&nbsp&nbsp<%=Utlitites.scale(entry.getValue().getExeCount(),entry.getValue().getExeCountBaseLine()) %></td>
	  <td align="center" width="60"><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=entry.getValue().getAverageKeyId() %>)"/><%=Utlitites.formatDotTwo(entry.getValue().getAverageExe()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=entry.getValue().getAverageKeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
...
<table id="OUT_HSF-Consumer" width="100%" border="1" class="ui-widget ui-widget-content">
	<tr class="headcon">
		<td align="left" colspan="11" width="100%"><font color="#000000" size="2">[ HSF ]</font>&nbsp;&nbsp;</td>
	</tr>	
	...
	for(Map.Entry<String,List<KeyValuePo>> entry:outHsfKeyMap.entrySet()){		
		...
		// 获取当前key当前时间的基线数据
		KeyValueBaseLinePo baseLinePo = MonitorBaseLineAo.get().findKeyBaseValueByDate(appInfopo.getAppId(), keyId, list.get(0).getCollectTime());
		String baseLineValue;
		if(baseLinePo != null) {
			baseLineValue = String.valueOf(baseLinePo.getBaseLineValue());
		} else {
		...
		if(keys[1].equals("HSF-Consumer-BizException")){
			...
			if(keyName.indexOf(Constants.COUNT_TIMES_FLAG)>-1){
				...
				po.setExeCountBaseLine(baseLineValue); // 增加执行次数的baseLine数据
			}
			if(keyName.indexOf(Constants.AVERAGE_USERTIMES_FLAG)>-1){
				...
				po.setAverageExeBaseLine(baseLineValue); // 增加执行平均时间的baseLine数据
			}										
	...
	while(sortOutMapit.hasNext()){
		...
		for(HsfClass hc:hsfClassList){
			...
			%>					
			<tr>
				<td align="center" ><%=aimShow?po.getAim():" - " %></td>			
				<td align="center" ><%=classShow?po.getHtmlClassName():" - " %></td>
				<td align="center" ><%=po.getHtmlMethodName() %></td>
				<td align="center"  ><%=po.getCollectTime() %></td>
				<td align="center"  ><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=po.getCountkeyId() %>)"/><%=Utlitites.fromatLong(po.getExeCount()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=po.getCountkeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
				<td align="center"  ><%=Utlitites.fromatLong(po.getExeCountBaseLine()) %>&nbsp&nbsp<%=Utlitites.scale(po.getExeCount(), po.getExeCountBaseLine()) %></td>			
				<td align="center"  ><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=po.getAverageKeyId() %>)"/><%=Utlitites.formatDotTwo(po.getAverageExe()) %>&nbsp;<a href='<%=request.getContextPath() %>/alarm/add_alarm_key.jsp?keyId=<%=po.getAverageKeyId() %>&appName=<%=appName %>' title='将此key加入告警' target="_blank">加</a></td>
				<td align="center"  ><%=Utlitites.fromatLong(po.getAverageExeBaseLine()) %>&nbsp&nbsp<%=Utlitites.scale(po.getAverageExe(), po.getAverageExeBaseLine()) %></td>
				<td align="center"  ><img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=po.getExcCountkeyId() %>)"/><%=po.getExecptionNum()==null?" - ":po.getExecptionNum() %>/<img src="<%=request.getContextPath () %>/statics/images/report.png" onClick="openTime(<%=po.getBizCountkeyId() %>)"/><%=po.getBizExecptionNum()==null?" - ":po.getBizExecptionNum() %></td>
