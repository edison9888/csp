...
<script type="text/javascript" src="<%=request.getContextPath() %>/statics/js/ui/jquery.ui.accordion.js"></script>
...
<div id="accordion">
	...
	%>
	<h3><a href="#"><%=appBo.getAppName() %>&nbsp;总量:[<%=Utlitites.fromatLong(appBo.getCallNum()+"") %>]&nbsp;来源:[<%=cmInfo %>]</a></h3>
	<div>
		<% 
		  long all_cm3 = 0;
		  long all_cm4 = 0;
		...
		<table width="1440" border="1">
		  ...
		  %>
		   <tr>
		  <%if(f){ %>
		    <td rowspan="<%=appBo.getKeyMap().size() %>" valign="top"><a href="./distrib_customer.jsp?opsName=<%=appBo.getAppName() %>"><%=appBo.getAppName() %></a></td>
		  <%f = false;} %>
		  	<%
		  		String[] keyNames = keyName.split("_");
