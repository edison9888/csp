...
%>
<form action="./distrib_customer.jsp" >
...
for(DistribFlowPo po:poList){
	String customerApp = po.getProviderApp();
...
<table width="1440" border="1">
  ...
  <tr>
    <td align="center">来源应用</td>
    <td height="20" align="center">来源方法</td>
  ...
  %>
   <tr>
  <%if(f){ %>
    <td rowspan="<%=appBo.getKeyMap().size() %>"><%=appBo.getAppName() %>(<%=Utlitites.fromatLong(appBo.getCallNum()+"") %>)</td>
  <%f = false;} %>
    <td><%=keyName.substring(22,keyName.length()) %></td>
