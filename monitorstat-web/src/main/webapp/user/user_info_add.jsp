...
<form id="f" name="f" action="./user_info_add.jsp" method="post" onsubmit="getNewListOptionsValue()">
...
if("add".equals(action)){
	...
	if(b) {
		response.setHeader("Refresh","0.01;URL=./user_info_add_relKey.jsp?userName=" + name + "&alarmGroups=" + gs);
	}
}else{
...
<ul>
	...
	%>
		<li><a href="#tabs-<%=info.getId() %>"><%=info.getName() %></a></li>
	<%
	}
	%>
...
<table width="100%">
	<tr>
		<td align="center"><input type="hidden" value="add" name="action"><input type="submit" value="下一步"><input type="button" value="关闭" onclick="window.close()"></td>
	</tr>
</table>
</form>
<%} %>
