...
<%@page import="sun.security.krb5.internal.HostAddress"%>
<%@page import="com.taobao.monitor.script.KeyScripRelation"%>
<%@page import="com.taobao.monitor.script.ao.ScriptAo"%>
<%@page import="com.taobao.monitor.script.ScriptlibraryInfo"%><html>
<head>
...
if("add".equals(action)){
	...
	String[] selectScriptIds = request.getParameterValues("selectId");
	List<KeyScripRelation> scriptRelList = new ArrayList<KeyScripRelation>();
	for(String s : selectScriptIds) {
		
		KeyScripRelation ksrel = new KeyScripRelation();
		ksrel.setAppId(Integer.parseInt(appId));
		ksrel.setKeyId(Integer.parseInt(keyId));
		ksrel.setPerformanceDefine("");
		ksrel.setScriptId(Integer.parseInt(s));
		scriptRelList.add(ksrel);
	}
	
	boolean b = ScriptAo.get().addKeyAndScriptRel(scriptRelList);
...
List<ScriptlibraryInfo> scriptLibraryList = ScriptAo.get().findAllScript();
List<KeyScripRelation> tempList = ScriptAo.get().findScriptRelByKeyIdAndAppId(Integer.parseInt(keyId), Integer.parseInt(appId));
List<Integer> selectedScriptIdList = new ArrayList<Integer>();

if(tempList != null) {
	for(KeyScripRelation s : tempList) {
		
		selectedScriptIdList.add(s.getScriptId());
	}
}
%>

<div class="ui-dialog ui-widget ui-widget-content ui-corner-all " style="width: 95%;">
<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">key关联脚本</div>
<div id="dialog">
...
<table id="example" border="1">
  ...
  <tbody>
	...
	for(ScriptlibraryInfo script : scriptLibraryList){

		if(!selectedScriptIdList.contains(script.getScriptId())) {
	%>
	<tr>
		<td align="center"><input type="checkbox" name='selectId' class='selectId' value="<%=script.getScriptId() %>"></td>
		<td align="center"><%=script.getScriptName() %></td>
	</tr>
