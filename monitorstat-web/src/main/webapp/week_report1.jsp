...
<table width="100%" border="1" width="100%" border="1" class="ui-widget ui-widget-content">
  ...
  needReportApp.add("sell");
  needReportApp.add("mytaobao");
  needReportApp.add("favorite");
  needReportApp.add("vist");
  needReportApp.add("shopstreet");
  needReportApp.add("magicroom");
  needReportApp.add("verticallist");
  needReportApp.add("tbskip");
  ...
  <tr>
	...
	<%	
		...
		double maxQps = MonitorLoadRunAo.get().findRecentlyAppLoadRunQps(AppCache.get().getKey(appPo.getAppName()).getAppId());
    %>    
    <td align="center">
    <%
  	 long num =  ((long)manage.findMaxKeyValue("PV"));
    if(num<1){
    	num = ((long)manage.findMaxKeyValue("PV_VISIT_COUNTTIMES"));
    }
    %>
    <%=Utlitites.fromatLong(num+"") %>
    </td>
    <td align="center"><%=manage.keyRisingTrendStr("PV") %></td>
    <td align="center"><%=Utlitites.formatDotTwo(manage.findMaxKeyValue("IN_QPS")) %>
    <%if(maxQps >0){
    	out.print("余量:"+Utlitites.formatDotTwo(maxQps-manage.findMaxKeyValue("IN_QPS")));
    } %>  
    </td>
