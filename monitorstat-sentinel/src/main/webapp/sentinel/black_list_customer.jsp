...
function add() {
	window.open ('manage.do?method=goToAddBlackListCustomer', 'newwindow', 'height=600, width=900, top=150, left=150')
}

function viewIps(refId, appName, customerInfo, refApp) {
	var urlAddr = "show.do?method=showBlackListCustomerIps&refId=" + refId + "&appName=" + appName + "&customerInfo=" + customerInfo + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=BLACK_LIST_CUSTOMER" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteBlackListCustomerConfig";
