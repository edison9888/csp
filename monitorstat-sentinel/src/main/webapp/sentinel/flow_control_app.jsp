...
<table class="bordered-table"  id="mytable" >
      <thead align="center">
      <tr align="center">
      	<th style="width:10%">${strategy}限流</th>
      	<th style="width:25%">应用名</th>
        <th style="width:25%">限流应用</th>
        <th style="width:6%">限流阀值</th>
        <th style="width:5%">状况</th>
        <th style="width:12%">过时标志</th>
...
function add() {
	window.open ('manage.do?method=goToAddFlowControlApp&strategy=${strategy}', 'newwindow', 'height=600, width=900, top=150, left=150')
}

function viewIps(refId, appName, refApp) {
	var urlAddr = "show.do?method=showFlowControlAppIps&refId=" + refId + "&appName=" + appName + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=FLOW_CONTROL_APP" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteFlowControlAppConfig";
...
function alter(id, appName) {
	...
	var parameters = "id=" + id + "&type=FLOW_CONTROL_APP" + "&thread=" + thread + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
...
function multiAlter(){
	...
	var parameters = "ids=" + ids + "&type=FLOW_CONTROL_APP" + "&thread=" + thread + "&appName=" + app;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
