...
function add() {
	window.open ('manage.do?method=goToAddWhiteListInterface', 'newwindow', 'height=600, width=900, top=150, left=150')
}


function viewIps(refId, appName, interfaceInfo, refApp) {
	var urlAddr = "show.do?method=showWhiteListInterfaceIps&refId=" + refId + "&appName=" + appName + "&interfaceInfo=" + interfaceInfo + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=WHITE_LIST_INTERFACE" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteWhiteListInterfaceConfig";
