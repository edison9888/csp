...
<table class="left_bordered-table"  id="mytable" >
    <tr>
    	<td colspan="2"><font size="5">新增自定义白名单</td>
	<tr>	
		<td>应用名:</td>
...
function add() {	
	...
	if (app_name==null || app_name=="" || customer_info==null || customer_info=="" || white_app==null || white_app=="") {
		alert("应用名、自定义关键字及白名单应用均不能为空");
		return;
	}
    var urlDest = "manage.do?method=addWhiteListCustomer"
    var parameters = "appName=" + app_name + "&customerInfo=" + customer_info + "&whiteApp=" + white_app;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
