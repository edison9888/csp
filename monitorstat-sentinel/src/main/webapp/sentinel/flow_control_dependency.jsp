...
<table class="bordered-table"  id="mytable" >
      <thead align="center">
      <tr align="center">
      	<th style="width:10%"><input type="checkbox" name="allSelect" id="allSelect" onclick="allSelect()"/>依赖限流</th>
      	<th style="width:8%">应用名</th>
        <th style="width:8%">限流应用</th>
        <th style="width:43%">接口信息</th>
        <th style="width:6%">线程数</th>
...
function add() {
	window.open ('manage.do?method=goToAddFlowControlDependency', 'newwindow', 'height=600, width=900, top=150, left=150');
}

function filter() {
	val = $("#index").val();
	var url = "show.do?method=showFlowControlDependency&index=" +  val;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=FLOW_CONTROL_DEPENDENCY" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteFlowControlDependencyConfig";
...
function alter(id, appName) {
	...
	var parameters = "id=" + id + "&type=FLOW_CONTROL_DEPENDENCY" + "&thread=" + thread + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
...
function multiAlter(){
	...
	var parameters = "ids=" + ids + "&type=FLOW_CONTROL_DEPENDENCY" + "&thread=" + thread + "&appName=" + app;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
