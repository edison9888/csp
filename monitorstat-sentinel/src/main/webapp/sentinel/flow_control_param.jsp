...
<table class="condensed-table">
 <tr>
 <td>
 <input type="text" id="index" value="" class="xlarge"/> <input type="button" value=" 参数关键字过滤  " onclick="filter()"/>
...
<table class="bordered-table"  id="mytable" >
      <thead align="center">
      <tr align="center">
      	<th style="width:10%"><input type="checkbox" name="allSelect" id="allSelect" onclick="allSelect()"/> 参数限流</th>
      	<th style="width:8%">应用名</th>
      	<th style="width:37%">参数信息</th>
      ...
      <tr>
      	...
      	<td align="center" style= "word-break:break-all">${o.paramInfo}</td>
...
function add() {
	window.open ('manage.do?method=goToAddFlowControlParam', 'newwindow', 'height=600, width=900, top=150, left=150');
}

function filter() {
	var val = $("#index").val();
	var url = "show.do?method=showFlowControlParam&index=" +  val;
...
function viewIps(refId, appName, refApp) {
	var urlAddr = "show.do?method=showFlowControlParamIps&refId=" + refId + "&appName=" + appName + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=FLOW_CONTROL_PARAM" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteFlowControlParamConfig";
...
function alter(id, appName) {
	var thread = window.prompt("请输入新的限流阀值");
	if (thread == null) {
		return;
	}
	var patrn=/^[0-9]{1,20}$/; 
	...
	var parameters = "id=" + id + "&type=FLOW_CONTROL_PARAM" + "&thread=" + thread + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
...
function multiAlter(){
	...
	var parameters = "ids=" + ids + "&type=FLOW_CONTROL_PARAM" + "&thread=" + thread + "&appName=" + app;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
