...
<table class="left_bordered-table"  id="mytable" >
    <tr>
    	<td colspan="2"><font size="5">新增接口黑名单</td>
	<tr>	
		<td>应用名:</td>
...
function add() {	
	...
	if (app_name==null || app_name=="" || interface_info==null || interface_info=="" || black_app==null || black_app=="") {
		alert("应用名、接口信息及黑名单应用均不能为空");
		return;
	}
    var urlDest = "manage.do?method=addBlackListInterface"
    var parameters = "appName=" + app_name + "&interfaceInfo=" + interface_info + "&blackApp=" + black_app;
    $.ajax({
    	url: urlDest,
    	async:false,
    	type: "POST",
