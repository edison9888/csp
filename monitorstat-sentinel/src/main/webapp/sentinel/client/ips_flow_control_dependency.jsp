...
function viewIps() {
	...
	var url = "info.do?method=ipsFlowControlDependency&appName=" + appName;
	window.open(url,"_self");
}
	
function efficientConfig(ip, appName) {
 	var urlAddr = "configFeedback.do?method=checkFlowControlDependency&ip=" + ip + "&appName=" + appName;
 	window.open (urlAddr, 'newwindow', 'scrollbars=yes, height=600, width=900, top=150, left=150');
}

function result(ip, appName) {
	var urlAddr = "resultFeedback.do?method=resultFlowControlDependency&ip=" + ip + "&appName=" + appName;
 	window.open (urlAddr, 'newwindow', 'scrollbars=yes, height=600, width=900, top=150, left=150');
}
