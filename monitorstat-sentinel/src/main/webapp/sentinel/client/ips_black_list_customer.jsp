...
<table class="condensed-table">
 ...
 <strong>黑名单关键字</strong>&nbsp;&nbsp;<select name="customerNames"  id="customerNames" class="xlarge"></select>&nbsp;&nbsp;
...
function change() {
	...
	var urlDest = "info.do?method=blackListCustomer";
...
function viewIps() {
	...
	var url = "info.do?method=ipsBlackListCustomer&appName=" + appName + "&customerInfo=" + customerInfo;
	window.open(url,"_self");
}
	
function efficientConfig(ip, customerInfo, appName) {
 	var urlAddr = "configFeedback.do?method=checkBlackListCustomer&ip=" + ip + "&customerInfo=" + customerInfo + "&appName=" + appName;
...
function result(ip, customerInfo, appName) {
	var urlAddr = "resultFeedback.do?method=resultBlackListCustomer&ip=" + ip + "&customerInfo=" + customerInfo + "&appName=" + appName;
 	window.open (urlAddr, 'newwindow', 'scrollbars=yes, height=600, width=900, top=150, left=150');
}
