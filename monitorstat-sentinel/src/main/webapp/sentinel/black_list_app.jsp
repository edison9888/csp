...
<table class="bordered-table"  id="mytable" >
      <thead align="center">
      <tr align="center">
      	<th style="width:10%">应用黑名单</th>
      	<th style="width:30%">应用名</th>
        <th style="width:30%">黑名单应用</th>
        <th style="width:5%">状况</th>
        <th style="width:10%">过时标志</th>
...
function viewIps(refId, appName, refApp) {
	var urlAddr = "show.do?method=showBlackListAppIps&refId=" + refId + "&appName=" + appName + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=BLACK_LIST_APP" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteBlackListAppConfig";
