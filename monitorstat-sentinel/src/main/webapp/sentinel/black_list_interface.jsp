...
<table class="bordered-table"  id="mytable" >
      <thead align="center">
      <tr align="center">
      	<th style="width:10%">接口黑名单</th>
      	<th style="width:10%">应用名</th>
        <th style="width:45%">接口信息</th>
        <th style="width:10%">黑名单应用</th>
      ...
      <tr>
      	...
      	<td>
      		<a href="#" onclick='viewIps("${o.id}", "${o.appName}", "${o.interfaceInfo}", "${o.refAppName}")'>查看ip &nbsp;</a>
...
function add() {
	window.open ('manage.do?method=goToAddBlackListInterface', 'newwindow', 'height=600, width=900, top=150, left=150')
}

function viewIps(refId, appName, interfaceInfo, refApp) {
	var urlAddr = "show.do?method=showBlackListInterfaceIps&refId=" + refId + "&appName=" + appName + "&interfaceInfo=" + interfaceInfo + "&refApp=" + refApp;
...
function changeState(id, operator, appName) {
    ...
    var parameters = "id=" + id + "&type=BLACK_LIST_INTERFACE" + "&appName=" + appName;
    $.ajax({
    	url: urlDest,
    	async:true,
    	type: "POST",
...
function deleteConfig(id, appName) {
	...
	var urlDest = "./manage.do?method=deleteBlackListInterfaceConfig";
