...
<script>
	$("#page_nav").load('<%=request.getContextPath()%>/page_nav.jsp', {urlPrefix:'/app/detail/notify/provider/show.do?method=showIndex&appId=',appId: '${appInfo.appId}'});
</script>	
</div>
	<div class="row-fluid">
        <div class="span2">
				...
				<table class="table table-striped table-bordered table-condensed"  width="100%">
					<thead>
						<tr>
							...
							<td style="text-align:center;">Key-3详情</td>
					...
					<tbody id="exctbody">
						...
						%>
							<tr>
								<td width="250" rowspan="2"  style="text-align:center;"><%=key2Name %><a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName=<%=se.getFullKeyName()%>' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>
								<%
								for(String time:timeList){ 
								...
								<td style="text-align:center;" rowspan="2"><a target='_blank' title='查看主机调用量' href='<%=request.getContextPath()%>/app/detail/notify/provider/show.do?method=showKey2HostInfo&appId=${appInfo.appId}&key1Name=${key1Name}&key2Name=<%=key2Name %>'>查看</a></td>
								<td style="text-align:center;" rowspan="2"><a target='_blank' title='查看Key-3调用量' href='<%=request.getContextPath()%>/app/detail/notify/provider/show.do?method=showKey3AppInfo&appId=${appInfo.appId}&key1Name=${key1Name}&key2Name=<%=key2Name %>'>查看</a></td>
								<td style="text-align:center;" rowspan="2"><a target='_blank' title='查看历史调用量' href='<%=request.getContextPath()%>/app/detail/history.do?method=showHistory&appName=${appInfo.appName}&keyName=<%=se.getFullKeyName() %>'>查看</a></td>
							</tr>
							<tr>
								<%
								for(String time:timeList){ 
