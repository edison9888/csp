...
->${appInfo.appName } 依赖的Notify   &nbsp;&nbsp;${group }分组的类型
<div class="row-fluid">
 <div class="span12">
  <table class="table table-striped table-bordered table-condensed" id="appNotifyConsumerGroupTable">
   <caption>
    <strong>${appInfo.appName } 依赖的notify ${group }分组的类型</strong>
   </caption>
   <thead>
      <tr>
   ...
   <tbody id="appNotifyConsumerGroupTableBody">
   </tbody>
  </table>
 </div>
</div>
...
function query(){
  ...
  $.ajax({
	   url : base+"/app/detail/notify/consumer/show.do?method=queryGroup&appId=${appInfo.appId}&key=${param.key}",
	   success : function(data) {
	    ...
	    $("#appNotifyConsumerGroupTableBody").html(tbody);
