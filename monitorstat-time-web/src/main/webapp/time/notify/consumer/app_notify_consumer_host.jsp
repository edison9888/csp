...
-><a href="<%=request.getContextPath()%>/app/detail/notify/consumer/show.do?method=showIndex&appId=${appInfo.appId}">${appInfo.appName}依赖的notify分组 </a>
->${appInfo.appName } 依赖的Notify   &nbsp;&nbsp;${group }分组的主机
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="table1">
			<caption>
				<strong>${appInfo.appName }主机依赖的notify ${group}分组</strong>
			</caption>
			<thead>
						<tr>
							<td width="200" style="text-align:center;">主机</td>
							...
							<td style="text-align:center;">类型详情</td>
...
function query(){
		...
		$.ajax({
			url : '<%=request.getContextPath()%>/app/detail/notify/consumer/show.do?method=queryHost&appId=${appInfo.appId}&key=${param.key}',
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td  style='text-align:center;' ><a href='<%=request.getContextPath()%>/app/detail/notify/consumer/show.do?method=gotoHostGroup&appId=${appInfo.appId}&key="+row.fullKeyName.replace("<%=Constants.S_SEPERATOR%>"+row.ip,"") +"&ip="+row.ip+"'>查看</a></td>";
