...
-><a href="<%=request.getContextPath()%>/app/detail/notify/consumer/show.do?method=gotoHost&appId=${appInfo.appId}&key=${param.key}">${appInfo.appName}主机依赖的notify${group }分组 </a>
->${param.ip }
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appNotifyConsumerHostGroupTable">
			<caption>
				<strong>${appInfo.appName }${param.ip }依赖的notify ${group}分组的接口和方法</strong>
			</caption>
			<thead>
						<tr>
...
function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/notify/consumer/show.do?method=queryHostGroup&appId=${appInfo.appId}&key=${param.key}&ip=${param.ip}",
