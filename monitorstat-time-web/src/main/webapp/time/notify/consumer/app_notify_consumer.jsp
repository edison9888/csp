...
<!-- <script type="text/javascript" 
			src="<%=request.getContextPath()%>/time/notify/consumer/AppNotifyConsumer.js"></script> -->
...
->${appInfo.appName } 依赖的Notify分组
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appNotifyConsumerTable">
			<caption>
				<strong>${appInfo.appName } 依赖的notify分组</strong>
			</caption>
			<thead>
						<tr>
							<td width="200" style="text-align:center;">分组</td>
			...
			<tbody id="appNotifyConsumerTableBody">
			</tbody>
		</table>
	</div>
</div>
...
function query(){
		...
		$.ajax({
			url : base+"/app/detail/notify/consumer/show.do?method=queryIndex&appId=${appInfo.appId}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td  style='text-align:center;' ><a href='"+base+"/app/detail/notify/consumer/show.do?method=gotoHost&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
					tr+="<td  style='text-align:center;' ><a href='"+base+"/app/detail/notify/consumer/show.do?method=gotoGroup&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
				...
				$("#appNotifyConsumerTableBody").html(tbody);
