...
<script>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			<a href="<%=request.getContextPath()%>/index.jsp">实时首页</a> -><a href="<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=${appInfo.appId}">${appInfo.appName} </a>->${appInfo.appName }的信息
			<hr>
			<div class="row-fluid">
				<table class="table table-striped table-bordered table-condensed"  width="100%">
					...
					<tbody id="exctbody">
						...
						%>
							<tr>
								<td style="text-align:center;" width="250" rowspan="5"><%=keyName %><a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName=<%=se.getFullKeyName()%>' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>
								<%
								for(String time:timeList){ 
								...
								<td style="text-align:center;" rowspan="5"><a target='_blank' title='查看主机调用量' href='<%=request.getContextPath()%>/app/detail/customer/show.do?method=showHostKey1Info&appId=${appInfo.appId}&keyName=<%=keyName %>'>查看</a></td>
								<td style="text-align:center;" rowspan="5"><a target='_blank' title='查看Key-2调用量' href='<%=request.getContextPath()%>/app/detail/customer/show.do?method=showKey2AppInfo&appId=${appInfo.appId}&key1Name=<%=keyName %>'>查看</a></td>
								<td style="text-align:center;" rowspan="5"><a target='_blank' title='查看历史调用量' href='<%=request.getContextPath()%>/app/detail/history.do?method=showHistory&appName=${appInfo.appName}&keyName=<%=KeyConstants.CHONGZHIDE+Constants.S_SEPERATOR+keyName %>'>查看</a></td>
							</tr>
							<tr>
								<%
								for(String time:timeList){ 
