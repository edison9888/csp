	...
	<div class="row-fluid">
		...
		<div class="span12">
			...
			-><a href="<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=${appInfo.appId}" >${appInfo.appName}</a> 
			 -><a href="<%=request.getContextPath()%>/app/detail/perf/show.do?method=showIndex&appId=${appInfo.appId}" >性能数据  首页 </a>
			 -> ${param.title}
			<hr>
			<div class="row-fluid">
				<h4> </h4>
				<table class="table table-striped table-bordered table-condensed"  width="100%">
					<thead>
						<tr>
							<td  style="text-align:center;">${param.title }</td>
							...
							<td style="text-align:center;" id="time10">历史</td>
						</tr>
					</thead>
					<tbody id="exctbody">
						<c:forEach items="${eMap }" var="ex">
							<tr id="tr_${ex.key }">
								...
								<td style="text-align:center;" id="${ex.key }_time11">0</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/perf/show.do?method=queryDetail&appId=${appInfo.appId}&key=${param.key}&mainProp=${param.mainProp}&title=${param.title}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td  style='text-align:center;'><a target='_blank' title='查看历史调用量' href='<%=request.getContextPath()%>/app/detail/history.do?method=showHistoryByip&appName="+row.appName+"&keyName="+row.keyName+"&ip="+row.ip+"'>查看</a></td></tr>";
					tbody+=tr;
				}
				$("#exctbody").html(tbody);
