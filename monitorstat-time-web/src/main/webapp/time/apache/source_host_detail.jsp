...
<script>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			<a href="<%=request.getContextPath()%>/index.jsp">实时首页</a> -><a href="<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=${appInfo.appId}" >${appInfo.appName}</a> -><a href="<%=request.getContextPath()%>/app/detail/apache/show.do?method=showIndex&appId=${appInfo.appId}">Web流量信息</a> -><a href="<%=request.getContextPath()%>/app/detail/apache/show.do?method=gotoSourceDetail&appId=${appInfo.appId}">自身URL-详细</a>->${url }&nbsp;主机列表
			<hr>
			<div class="row-fluid">
				<h4>主机列表</h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/apache/show.do?method=querySourceHostDetail&appId=${appInfo.appId}&key=${key}",
					...
					for(var i=10;i>=1;i--){
						...
						if(tmp[k]){
							...
							for(var prop in propertyMap){
								proptr +="<tr><td>"+ prop +"</td>";	
								proptr +="<td>"+  propertyMap[prop] +"</td></tr>"
							}
							h += proptr+"</table>";
