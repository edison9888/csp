...
<script>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			<a href="<%=request.getContextPath()%>/index.jsp">实时首页</a> -><a href="<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=${appInfo.appId}" >${appInfo.appName}</a> -><a href="<%=request.getContextPath()%>/app/detail/apache/show.do?method=showIndex&appId=${appInfo.appId}">Web流量信息</a> ->被调用URL-详细
			<hr>
			<div class="row-fluid">
				<h4>被调用URL - 详细</h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/apache/show.do?method=queryReferDetail&appId=${appInfo.appId}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					var tr="<tr id='tr_"+n+"'><td >"+ (row.referAppName) +"</td>";
					tr +="<td>"+row.keyName+"<a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName="+row.fullKeyName+"' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>";
					...
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/apache/show.do?method=gotoReferHostDetail&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
