...
-><a href="<%=request.getContextPath()%>/app/detail/search/show.do?method=showIndex&appId=${appInfo.appId}">搜索 首页 </a>
->${url }
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appSearchHostTable">
			<caption>
				<strong>${appInfo.appName }主机依赖的搜索URL</strong>
			</caption>
			<thead>
						<tr>
							<td width="200" style="text-align:center;">IP</td>
...
function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/search/show.do?method=searchHost&appId=${appInfo.appId}&key=${param.key}",
