...
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appTairConsumerTable">
			<caption>
				<strong>${appInfo.appName } 依赖的tair分组</strong>
			</caption>
			<thead>
						<tr>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/tair/show.do?method=queryGroup&appId=${appInfo.appId}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoGroupHost&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoNamespace&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
