...
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appTairConsumerHostTable">
			<caption>
				<strong>${appInfo.appName }主机依赖的tair ${group}分组</strong>
			</caption>
			<thead>
						<tr>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/tair/show.do?method=queryGroupHost&appId=${appInfo.appId}&key=${param.key}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td  style='text-align:center;' ><a href='<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoGroupHostNamespace&appId=${appInfo.appId}&key="+row.fullKeyName.replace("<%=Constants.S_SEPERATOR%>"+row.ip,"")+"&ip="+row.ip+"'>查看</a></td>";
