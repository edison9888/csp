...
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appTairHostConsumerTable">
			<caption>
				<strong>${appInfo.appName }主机 提供的Tair</strong>
			</caption>
			<thead>
						<tr>
...
function query(){
		...
		$.ajax({
			url : '<%=request.getContextPath()%>/app/detail/tairProvider/show.do?method=queryHost&appId=${appInfo.appId}',
					...
					for(var i=10;i>=1;i--){
						...
						if(tmp[k]){
							...
							for(var prop in propertyMap){
								h+="<tr>"+"<td>"+ prop +"</td>"+"<td>"+  propertyMap[prop] +"</td>"+"</tr>";
							}
							h += "</table>";
