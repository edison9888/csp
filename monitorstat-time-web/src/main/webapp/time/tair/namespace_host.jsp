		...
		<%--导航 --%>
			...
			<c:forEach var="item" items="${fn:split(param.key,  s_sperator)}" varStatus="status">
				...
				<c:if test="${status.index==2}">
					<c:set var="namespace" value="${item}"/>
				</c:if>
			</c:forEach>
			...
			->  <a href="<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoGroup&appId=${appInfo.appId}">分组列表</a>
			-><a href="<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoNamespace&appId=${appInfo.appId}&key=<%=KeyConstants.TAIR_CONSUMER+ Constants.S_SEPERATOR %>${group}">${group} 命名空间和方法列表 </a>
			->主机列表
			<hr>
			<div class="row-fluid">
				<h4>来源应用调用 </h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/tair/show.do?method=queryNamespaceHost&appId=${appInfo.appId}&key=${param.key}",
