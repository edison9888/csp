...
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appTairHostGroupTable">
			<caption>
				<strong>${appInfo.appName } ${ip }依赖的tair分组</strong>
			</caption>
			<thead>
						<tr>
							...
							<td style="text-align:center;">命名空间详情</td>
...
function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/tair/show.do?method=queryHostGroup&appId=${appInfo.appId}&ip=${param.ip}",
			success : function(data) {
		     ...
		     tr+="<td  style='text-align:center;' ><a href='<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoHostGroupNamespace&appId=${appInfo.appId}&ip=${param.ip}&key="+row.fullKeyName+"'>查看</a></td>";
