...
-><a href="<%=request.getContextPath()%>/app/detail/tair/show.do?method=gotoNamespace&appId=${appInfo.appId}&key=${param.ns}">Tair分组 ${group }  </a>
->Tair分组 ${param.key}  方法
<div class="row-fluid">
	<div class="span12">
		<table class="table table-striped table-bordered table-condensed" id="appTairHostGroupNameSpaceTable">
			<caption>
				<strong>Tair分组 ${group }  方法</strong>
			</caption>
			<thead>
						<tr>
							<td width="200" style="text-align:center;">方法</td>
...
function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/tair/show.do?method=queryActions&appId=${appInfo.appId}&key=${param.key}",
