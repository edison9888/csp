...
<script>
	$("#page_nav").load('<%=request.getContextPath()%>/page_nav.jsp', {urlPrefix:'<%=request.getContextPath()%>/app/detail/tbsession/show.do?method=showIndex&appId=',appId: '${appInfo.appId}'});
</script>	
		...
		<div class="span12">
			...
			<div class="row-fluid">
				<table class="table table-striped table-bordered table-condensed"  width="100%">
					<thead>
						<tr>
							<td width="250" style="text-align:center;">操作类型</td>
					...
					<tbody id="exctbody">
						...
						<%
							for(SortEntry<TimeDataInfo> se:sortEntryList){
								String operStr = se.getKeyName();
						%>
							<tr>
								<td style="text-align:center;" width="250"><%=operStr %><a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName=<%=se.getFullKeyName()%>' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>
								<%
								for(String time:timeList){ 
								...
								<td style="text-align:center;"><a target='_blank' title='查看主机调用量' href='<%=request.getContextPath()%>/app/detail/tbsession/show.do?method=showHostInfo&appId=${appInfo.appId}&keyName=<%=operStr %>'>查看</a></td>
