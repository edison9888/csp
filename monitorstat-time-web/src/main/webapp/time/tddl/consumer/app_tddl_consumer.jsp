...
<script>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			...
			->${appInfo.appName } 消耗的TDDL信息
			<hr>
			<%
			if (sortEntryList == null || sortEntryList.size() == 0) {
				out.print("<h1>此应用无TDDL数据</h1>");
				return;
			}
			%>
			<div class="row-fluid">
				<table class="table table-striped table-bordered table-condensed"  width="100%">
					<thead>
						<tr>
							<td width="125" style="text-align:center;">数据库名</td>
							<td width="125" style="text-align:center;">数据库IP</td>
							<%for(String time:timeList){ %>
							<td style="text-align:center;"><%=time %></td>
							<%} %>
							<td style="text-align:center;">主机详情</td>
							<td style="text-align:center;">SQL详情</td>
					...
					<tbody id="exctbody">
						...
						<%
							for(SortEntry<TimeDataInfo> se:sortEntryList){
								String dbFeature = se.getKeyName();
								String[] values = dbFeature.split("\\$");
								String dbName = values[2];
								String dbIp = values[0];
						%>
							<tr>
								<td style="text-align:center;" width="125" rowspan="2"><%=dbName %><a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName=<%=se.getFullKeyName()%>' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>
								<td style="text-align:center;" width="250" rowspan="2"><%=dbIp %></td>
								<%
								for(String time:timeList){ 
								...
								<%}else{
									%>
									<td style="text-align:center;"><%=td.getOriginalPropertyMap().get("E-times")%></td>
									<%
									}} %>
								<td style="text-align:center;" rowspan="2"><a target='_blank' title='查看主机调用量' href='<%=request.getContextPath()%>/app/detail/tddl/consumer/show.do?method=showHostDBInfo&appId=${appInfo.appId}&keyName=<%=dbFeature %>'>查看</a></td>
								<td style="text-align:center;" rowspan="2"><a target='_blank' title='查看SQL调用量' href='<%=request.getContextPath()%>/app/detail/tddl/consumer/show.do?method=showDBSQLInfo&appId=${appInfo.appId}&keyName=<%=dbFeature %>'>查看</a></td>
							...
							<tr>
								...
								for(String time:timeList){ 
									...
									if (td == null) {
										...
										%>
										<td style="text-align:center;"><%=td.getOriginalPropertyMap().get("C-time") %>ms</td>
										<%
									}
								}
								%>
