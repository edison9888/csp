...
<body>
	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			...
			<div class="span12">
				<a href="<%=request.getContextPath()%>/index.jsp">实时首页</a> -> 应用导航操作
				&nbsp;${userName}
				<hr>
				<div class="row-fluid">
					<div class="span12">
						<div class="row-fluid">
							<form id="form_add" method="post"
								action="<%=request.getContextPath()%>/app/detail/custom/show.do?method=addNaviApp">
							...
							<form id="form2" method="post"
								action="<%=request.getContextPath()%>/app/detail/custom/show.do?method=deleteNaviApp&appId=${appInfo.appId}">
								<table
									class="table table-striped table-bordered table-condensed">
									<thead>
										<tr>
											<td><input type="checkbox" id="cb1" /> 全选</td>
											<td width="250" style="text-align: center;">导航(双击修改)</td>
											<td style="text-align: center;" id="time1">编辑</td>
										</tr>
									</thead>

									<tbody id="exctbody">
										<c:forEach items="${navisapp }" var="item">
											<tr>
												<td><input type="checkbox" name="naviId"
													value="${item.naviId }" /></td>
												<td width="250" class="edittd" id="${item.naviId }">${item.naviName }</td>
												<td style="text-align: center;"><a
													href="<%=request.getContextPath()%>/app/detail/custom/show.do?method=indexNaviKeyApp&naviId=${item.naviId }&naviName=${item.naviName}&userName=${userName}&appId=${appInfo.appId}">编辑导航</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
