...
<script>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			<a href="<%=request.getContextPath()%>/index.jsp">实时首页</a> -><a href="<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=${appInfo.appId}" >${appInfo.appName}</a>  -><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProviderIndex&appId=${appInfo.appId}" >HSf Provider 流量信息 </a>-> 来源应用调用
			<hr>
			<div class="row-fluid">
				<h4>来源应用调用 </h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
		//
			url : "<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=queryClassOrMethodUser&appName=${appInfo.appName}&key=${param.key}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfReferHost&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfReferClass&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
