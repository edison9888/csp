...
<script>
	...
	<div class="row-fluid">
		...
		<%--导航 --%>
			...
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/consumer/show.do?method=gotohsfConsumerClass&appId=${appInfo.appId}&key=<%=KeyConstants.HSF_CONSUMER%><%=Constants.S_SEPERATOR %>${referAppName}">${referAppName}  接口列表 </a>
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/consumer/show.do?method=gotohsfConsumerMethod&appId=${appInfo.appId}&key=<%=KeyConstants.HSF_CONSUMER%><%=Constants.S_SEPERATOR %>${referAppName}<%=Constants.S_SEPERATOR %>${className}">${className }方法列表 </a> 			
			->${methodName }主机列表 <hr>
			<div class="row-fluid">
				<h4>来源应用调用 </h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/hsf/consumer/show.do?method=queryhsfConsumerMethodHost&appId=${appInfo.appId}&key=${param.key}",
