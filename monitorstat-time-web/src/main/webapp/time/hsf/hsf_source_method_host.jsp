...
<script>
	...
	<div class="row-fluid">
		...
		<%--导航 --%>
			...
			->  <a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProvider&appId=${appInfo.appId}">提供的接口访问信息</a>
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProviderChlid&appId=${appInfo.appId}&key=<%=KeyConstants.HSF_PROVIDER %><%=Constants.S_SEPERATOR %>${className}">${className} 方法列表 </a>
			->${methodName } 主机列表
			<hr>
			<div class="row-fluid">
				<h4> </h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=queryhsfSourceMethodHost&appId=${appInfo.appId}&key=${param.key}",
