		...
		<%--导航 --%>
			...
			<c:forEach var="item" items="${fn:split(param.key,  s_sperator)}" varStatus="status">
				...
				<c:if test="${status.index==3}">
					<c:set var="methodName" value="${item}"/>
				</c:if>
			</c:forEach>
			...
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProviderIndex&appId=${appInfo.appId}" >HSf Provider 流量信息 </a>
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfRefer&appId=${appInfo.appId}">来源应用调用</a>
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfReferClass&appId=${appInfo.appId}&key=<%=KeyConstants.HSF_REFER%><%=Constants.S_SEPERATOR %>${referAppName}">${referAppName}  接口列表 </a>
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfReferMethod&appId=${appInfo.appId}&key=<%=KeyConstants.HSF_REFER%><%=Constants.S_SEPERATOR %>${referAppName}<%=Constants.S_SEPERATOR %>${className}">${className }方法列表 </a> 			
			->${methodName }主机列表 <hr>
			<div class="row-fluid">
				<h4>来源应用调用 </h4>
...
<script type="text/javascript">
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=queryhsfReferMethodHost&appId=${appInfo.appId}&key=${param.key}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					tr+="<td  style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/history.do?method=showHistoryByip&appName=${appInfo.appName}&keyName="+row.keyName+"&ip="+row.ip+"' target='_blank'>查看</a></td></tr>";
					tbody+=tr;
				}
				$("#exctbody").html(tbody);
