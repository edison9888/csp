...
%>
	...
	<div class="row-fluid">
		...
		<div class="span10">
			...
			-><a href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProvider&appId=${appInfo.appId}">接口详情</a>
			->${keyName}
			<hr>
			<div class="row-fluid">
			使用接口${keyName}使用信息 
					...
					<tbody id="exctbody">
						...
						<%
							for(SortEntry<TimeDataInfo> se:sortEntryList){
								String operStr = se.getFullKeyName().split(Constants.S_SEPERATOR)[1];
						...
						%>
							<tr>
								...
								<td style="text-align:center;"><a target='_blank' title='查看主机调用量' href='<%=request.getContextPath()%>/app/detail/tbsession/show.do?method=showHostInfo&appId=${appInfo.appId}&keyName=<%=se.getFullKeyName() %>'>查看</a></td>
