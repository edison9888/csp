...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/time/app_index.js"></script>
...
<script>
	$("#page_nav").load('<%=request.getContextPath()%>/page_nav.jsp', {urlPrefix:'<%=request.getContextPath()%>/app/detail/show.do?method=showIndex&appId=',appId: '${appInfo.appId}'});
			...
			<div class="row-fluid">
				...
				<div class="span12">
					...
					<c:forEach var="item" items="${fn:split(param.key,  s_sperator)}"
						varStatus="status">
					...
					->
					<a
						href="<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfProvider&appId=${appInfo.appId}">提供的接口访问信息</a>
					->${className}方法列表
					<hr>
					<div class="row-fluid">
						<h4>
							接口${className}全网调用量信息
						</h4>
						<table class="table table-striped table-bordered table-condensed"
							...
							<thead>
								<tr>
									...
									<td style="text-align: center;" id="time4">
										10:24
									</td>
									<td style="text-align: center;" id="time5">
										10:25
									</td>
									<td style="text-align: center;" id="time6">
										10:26
									</td>
									<td style="text-align: center;" id="time7">
										10:27
									</td>
									<td style="text-align: center;" id="time8">
										10:28
									</td>
									<td style="text-align: center;" id="time9">
										10:29
									</td>
									<td style="text-align: center;" id="time10">
										10:30
									</td>
							...
							<tbody id="exctbody">
								<c:forEach items="${eMap }" var="ex">
									<tr id="tr_${ex.key }">
										...
										<td style="text-align: center;" id="${ex.key }_time1">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time2">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time3">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time4">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time5">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time6">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time7">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time8">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time9">
											0
										</td>
										<td style="text-align: center;" id="${ex.key }_time10">
											0
										</td>
										<td style="text-align: center;">
											<a href="">查看</a>
	...
	function query(){
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=queryHsfProviderChild&appId=${appInfo.appId}&key=${param.key}",
			success : function(data) {
				...
				for(var n=0;n<data.length;n++){
					...
					var tr="<tr id='tr_"+n+"' ><td><a title='查看接口依赖路径' target='_blank' href='<%=request.getContextPath()%>/app/depend/query/show.do?method=queryKeyDetailWithTimeData&appName=${appInfo.appName}&keyName="+row.fullKeyName+"'>"+row.keyName+"</a><a target='_blank' href='<%=request.getContextPath()%>/app/conf/key/show.do?method=keyPropsList&&appId=${appInfo.appId}&keyName="+row.fullKeyName+"' ><img src='<%=request.getContextPath()%>/statics/img/add.png' width='10px' height='10px' title='加入告警' /></a></td>";
					for(var i=10;i>=1;i--){
						var k = tarray[(i-1)]
						if(tmp[k]){
					...
					tr+="<td style='text-align:center;'><a href='<%=request.getContextPath()%>/app/detail/hsf/provider/show.do?method=gotohsfSourceMethodHost&appId=${appInfo.appId}&key="+row.fullKeyName+"'>查看</a></td>";
