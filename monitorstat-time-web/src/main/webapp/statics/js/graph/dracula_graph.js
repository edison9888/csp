 ...
 */
Graph.Renderer.Raphael = function(element, graph, width, height,jsPlumb,jsPlumbConfig) {
    this.width = width || 400;
    this.height = height || 400;
    var selfRef = this;
    this.r = Raphael(element, this.width, this.height);
    this.jspb = jsPlumb;
    //jspb.Defaults.Anchors =  [ "BottomCenter", "TopCenter" ];
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function(node) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var ellipse = r.rect(0, 0, $("#"+node.id).width(),$("#"+node.id).height()+10).attr({fill: color, stroke: color, "stroke-width": 2});
                /* set DOM node ID */
                ellipse.node.id =  node.id+"_R";
                shape = r.set().
                    push(ellipse).
                    push(r.text(0, 30, node.id+"_R"));
    ...
    drawEdge: function(edge) {
    	...
    	//防止出现脏数据，必须保证父子节点的div都存在
    	if(objSource.length>0 && objTarget.length>0) {
        	this.jspb.connect({
        		source : sourceId,
        		target : targetId
        	},
        	edge.jsConfig||this.jsPlumbConfig);     		
    	}
    }
};
Graph.Layout = {};
