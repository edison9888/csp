...
import com.taobao.csp.time.custom.arkclient.authen.entity.CodeDefine;
import com.taobao.csp.time.custom.arkclient.authen.entity.SecretKeyIV;
import com.taobao.csp.time.custom.arkclient.authen.entity.UserProfile;
import com.taobao.csp.time.custom.arkclient.service.ArkAuthenService;
import com.taobao.csp.time.custom.arkclient.service.CodeEncryptService;
import com.taobao.csp.time.custom.arkclient.service.EncryptService;
import com.taobao.csp.time.custom.arkclient.service.IdEncryptService;
import com.taobao.csp.time.custom.arkclient.service.RequestService;

public class ArkAuthenRequest {
	private static Logger logger = Logger.getLogger(ArkAuthenRequest.class);
	...
	public ArkAuthenRequest(HttpServletRequest request, HttpServletResponse response) throws JSONException, RedirectException {
		...
		if (httpcookie != null && StringUtils.isNotBlank(httpcookie.getValue())) {
			...
			//"true".equals(request.getParameter("remote_debug"));
			SecretKeyIV secretkeyiv = getSecretKeyIv(request, secretid);
			logger.warn("secretkeyiv-1: "+secretkeyiv);
			if (secretkeyiv == null) {
				//获得过期时一致的缓存内容
				secretkeyiv = requestSecretKeyIv(secretid);
				logger.warn("secretkeyiv-2: "+secretkeyiv);
