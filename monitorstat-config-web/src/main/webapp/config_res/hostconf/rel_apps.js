...
function rel_app_add() {
	...
	function relAppAddUnit() {
		// url  这里传参数，主要是“添加应用”页面使用的
		var $url = "../../hostconf/no_rel_apps.do?serverId=" + page.serverId;
	...
	function relAppAddProcess() {
		...
		function popInfo(data) {
			...
			setTimeout(function() {
						...
						// 刷新页面
						//alert("刷新页面");
						window.location.href = "../../config/hostconf/rel_apps.jsp?id="+ page.serverId;
					}, 2000);
		...
		function sendReq() {
			...
			var url = "../../hostconf/rel_app_add.do";
...
function fillServerInfo() {
	...
	var ids = ["serverName", "serverIp", "serverDesc"];
	$.ajax({
				url : "../../hostconf/get.do?id=" + page.serverId,
...
function renderlist(data) {
	...
	// 从table tr th中获取。
	// 示例： ["serverName", "type1num", "serverUrl", "serverType", "operate"]
	var colNames = [];
...
function addEvents(td, id, tr) {
	if (td.className == "so_operate") {
		var links1 = {
			delete_1 : "../../hostconf/rel_app_delete.do?id=" + id + "&serverId="
					+ page.serverId
		}
...
function showlist() {
	$.ajax({
				url : "../../hostconf/rel_apps.do?id=" + page.serverId,
