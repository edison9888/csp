...
TINY.box = function() {
	...
	return {
		...
		/**
		 *@param c: content
		 *	1、可以是字符串、url、img、flash等等
		 *@param w、h
		 *	2、表示宽高，单位是px
		 *		'blank-for-test.html',1,300,150,1
		 * @param t
		 * 		表示时间，单位是秒
		 */
		...
		twalpha : function(e, a, d, s) {
			...
			if (o == a) {
				...
				if (d == -1) {
					...
					e == p
							? TINY.box.alpha(m, -1, 0, 3)
							: b.innerHTML = p.style.backgroundImage = ''
				} else {
					e == m ? this.alpha(p, 1, 100, 5) : TINY.box.fill(ic, iu,
							iw, ih, ia)
				}
			} else {
		...
		size : function(e, w, h, s) {
			...
			clearInterval(e.si);
			var ow = e.offsetWidth, oh = e.offsetHeight, wo = ow
					- parseInt(e.style.width), ho = oh
		...
		twsize : function(e, w, wo, wd, h, ho, hd, s) {
			...
			if (ow == w && oh == h) {
				...
				if (ow != w) {
					e.style.width = ow + (Math.ceil(Math.abs(w - ow) / s) * wd)
							+ 'px'
				}
				if (oh != h) {
					e.style.height = oh
							+ (Math.ceil(Math.abs(h - oh) / s) * hd) + 'px'
				}
				this.pos()
			}
		}
...
TINY.page = function() {
	return {
		...
		theight : function() {
			...
			return Math.max(Math.max(b.scrollHeight, e.scrollHeight), Math.max(
							b.clientHeight, e.clientHeight))
		},
		twidth : function() {
			var d = document, b = d.body, e = d.documentElement;
			return Math.max(Math.max(b.scrollWidth, e.scrollWidth), Math.max(
