...
import com.taobao.csp.hadoop.job.gclog.utils.DateUtil;

public abstract class GCParser
{
	private static Logger logger = Logger.getLogger(GCParser.class);
	...
	protected void add_logTime(GCMetric metric, String logTime) {
		gcstats().add(metric, logTime);
	}
...
// 	}
	...
	add_young_gc(double timestamp_beg,
		...
		double pause_time, String line)
	{
		String logTime;
		try {
			logTime = DateUtil.getDateFromLine(line) + "";
		} catch (Exception e) {
			logger.error("", e);
			logTime = "-1";
		}
		...
		add_logTime(GCMetric.ygc_logtime, logTime);
	...
	add_young_gc(Matcher m, int timestamp_grp,
		...
		int pause_time_grp, String line)
	{
		...
		double pause_time, String line)
	{
		...
		add_logTime(GCMetric.fgc_logtime, logTime);
	...
	protected void add_full_gc(double timestamp_beg,
		...
		double pause_time, String logTime)
	{
	...
	protected void add_full_gc(Matcher m, int timestamp_grp,
		...
		int th_used_beg_grp, int th_used_end_grp, int th_commit_end_grp,
		int pause_time_grp, String logTime)
	{
