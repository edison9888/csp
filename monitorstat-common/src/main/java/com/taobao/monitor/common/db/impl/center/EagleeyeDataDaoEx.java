...
public class EagleeyeDataDaoEx extends MysqlRouteBase {
	private static final Logger logger = Logger.getLogger(EagleeyeDataDaoEx.class);

	public static final long LIMIT_SIZE = 1000; 
	public EagleeyeDataDaoEx() {
		super(DbRouteManage.get().getDbRouteByRouteId("csp_dependent"));
	}

	public List<CspCallsRelationship> findCallsRelationship(String sourceUrl, String sourceApp){
		String sql = "select * from csp_calls_relationship where source_url=? and source_app=?";
		final List<CspCallsRelationship> list = new ArrayList<CspCallsRelationship>();
		try {
			this.query(sql, new Object[]{sourceUrl,sourceApp}, new SqlCallBack() {
				@Override
	...
	private void fillPo(ResultSet rs, CspCallsRelationship er) throws SQLException {
		...
		er.setType(rs.getInt("type"));
	}
	public Set<String> findExistKeyFromCallsRelationship(Set<String> set){

		Object[] params = new Object[set.size()];
		StringBuilder sb = new StringBuilder("select distinct origin from csp_calls_relationship where origin in(");
		int i=0;
		for(String key: set) {
			sb.append("?,");
			params[i++] = key;
		...
		final Set<String> setReturn = new HashSet<String>();

		try {
			this.query(sql, params, new SqlCallBack() {
				@Override
				public void readerRows(ResultSet rs) throws Exception {
					setReturn.add(rs.getString("origin"));
				}
			});
		} catch (Exception e) {
			logger.error("findCallsRelationship", e);
