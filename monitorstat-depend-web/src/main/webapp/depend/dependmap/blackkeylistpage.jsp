	...
	if(keyList == null) {
		...
		for(CspMapKeyInfoPo po : keyList) {
			...
			<tr class="<%=rowClass[keyLevel]%>">
				<td><%=po.getKeyname()%>&nbsp;
				<a href="<%=request.getContextPath()%>/show/dependmap.do?method=gotoMeDependKeyPage&appName=<%=appName%>&keyName=<%=po.getKeyname()%>" target="_blank;">看拓扑</a>
				</td>
				<td><%=po.getRate()%></td>
				...
				<td><a href="#" onclick="removeBlack(<%=po.getId()%>)">移除黑名单</a></td>
