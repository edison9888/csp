...
<%@ include file="../../../header.jsp"%>
	<form id="mainForm"  action="<%=request.getContextPath() %>/show/hsfconsume.do" method="get">
		...
		<div class="container">
			<div class="row">
				<div class="span12"><span style="font-weight:bold">&nbsp;&nbsp;<%=opsName%>依赖应用的列表</span></div>
			</div><!-- row -->
			<div class="row">
				<div class="span12"><%
																...
																<tr>
																	...
																	<td >
																		<a href="javascript:openWin('<%=request.getContextPath()%>/show/hsfconsume.do?method=showAppProviderDetail&providename=<%=is.getOpsName()%>&consumeName=<%=opsName%>&selectDate=<%= selectDate%>',700,1100)">接口分布</a>&nbsp;
																		<a href="javascript:openWin('<%=request.getContextPath()%>/show/hsfconsume.do?method=showAppConsumeProvideMachine&providename=<%=is.getOpsName()%>&consumeName=<%=opsName%>&selectDate=<%= selectDate%>',700,1100)">机器分布</a>&nbsp;
																		<a href="javascript:openWin('<%=request.getContextPath()%>/show/hsfconsume.do?method=showConsumeAppHistoryGraph&targetAppName=<%=is.getOpsName()%>&sourceAppName=<%=opsName%>&endDate=<%= selectDate%>',700,1100)">查看历史走势</a>
																			</td>
																</tr>
																<% }%>
																</table></div>
