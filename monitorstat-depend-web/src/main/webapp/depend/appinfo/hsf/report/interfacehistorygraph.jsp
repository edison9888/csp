...
<html>
	...
	<%
	  ...
	  if ("consume".equals(type)) {
	    ...
	    dataUrl2 += "/show/hsfconsume.do?method=getInterfaceHistoryGraphData&startDate="
	        + startDate
	        + "&endDate="
	        + endDate
	        + "&sourceAppName="
	        + sourceAppName + "&interfaceName=" + interfaceName;
	    
	    formSubUrl += "/show/hsfconsume.do?method=showInterfaceHistoryGraph";
	  } else {
	    ...
	    dataUrl2 += "/show/hsfprovide.do?method=getInterfaceHistoryGraphData&startDate="
	        + startDate
	        + "&endDate="
	        + endDate
	        + "&sourceAppName="
	    ...
	    formSubUrl += "/show/hsfprovide.do?method=showInterfaceHistoryGraph";
	  }
	...
	<input type="hidden" value="<%=interfaceName%>" name="interfaceName" id="interfaceName">
	<table>
		<tr>
	...
	<div align="center">
		<%
		  if ("consume".equals(type)) {
		    %>
		    <h5>应用${sourceAppName}调用接口<%=interfaceName%>的历史情况</h5>
		    <%
		  } else {
		    %>
		      <h5>应用${sourceAppName}的接口<%=interfaceName%>的HSF调用量</h5>
		    <%
		  }
		%>
