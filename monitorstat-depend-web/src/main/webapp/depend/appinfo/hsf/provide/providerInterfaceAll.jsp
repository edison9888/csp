...
<%@ include file="../../../header.jsp"%>
	<form id="mainForm"  action="<%=request.getContextPath() %>/show/hsfprovide.do" method="get">
		...
		<div class="container">
			<div class="row">
				<div class="span12"><span style="font-weight:bold">&nbsp;&nbsp;--<%= opsName%>--提供的HSF接口服务列表(本页对比数据的时间为查询时间的前7天)</span></div>
			</div><!-- row -->
			<div class="row">
				<div class="span12">		<%
																...
																<tr>
																	...
																	<td >
																	<a href="javascript:openWin('<%=request.getContextPath()%>/show/hsfprovide.do?method=showAppHsfInterfaceDetail&interfaceName=<%=is.getName() %>&opsName=${opsName}&selectDate=${selectDate}',700,1100)">应用分布</a>&nbsp;
																	<a href="javascript:openWin('<%=request.getContextPath()%>/show/hsfprovide.do?method=showProvideMachine&interfaceName=<%=is.getName() %>&opsName=${opsName}&selectDate=${selectDate}',700,1100)">机器分布</a>&nbsp;
