## 此脚本放在login
read -p '确认执行时间段（不在"2点-3点",不在后半小时），执行请输入Y:' time_confirm
if [ $time_confirm != 'Y' ];then
	exit;
fi
...
export LANG=C
pass="99lzhlmcl*1"
online_hosts=(coreperf144088.cm6)
auto_scp () {
...
checkoutPath=monitorstat/monitorstat-depend-web
