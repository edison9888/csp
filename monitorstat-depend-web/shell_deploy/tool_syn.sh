...
# daily更新git
gitDir=~/monitorstat/monitorstat-depend-web
auto_scp $pass tool_syn_helper.sh $daily_user@${daily_ip}:~
auto_ssh $pass $daily_user@$daily_ip sh tool_syn_helper.sh $gitDir
# 从daily拉脚本
auto_scp  $pass  $daily_user@${daily_ip}:$gitDir/shell_deploy/* .
