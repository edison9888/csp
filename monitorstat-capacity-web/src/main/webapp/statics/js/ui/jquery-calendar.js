	...
	showCalendar: function() {
		...
		if (this.timeSeparators.length > 1) {
			...
			if (this.timeSeparators.length == 4) {
				...
				for (var a = 0; a<24; a++) {
					if (this.selectedHour == a) timeSelect += '<option value="' + ((a >= 10) ? a : ('0' + a)) + '" selected="selected">' + ((a >= 10) ? a : ('0' + a))  + '</option>';
					else timeSelect += '<option value="' + ((a >= 10) ? a : ('0' + a)) + '">' + ((a >= 10) ? a : ('0' + a)) + '</option>';
				}
			}
