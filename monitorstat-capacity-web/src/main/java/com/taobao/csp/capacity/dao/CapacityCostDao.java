...
public class CapacityCostDao extends MysqlRouteBase {
	
	public static Logger logger = Logger.getLogger(CapacityCostDao.class);
	
	public CapacityCostDao(){
	...
	public List<CapacityCostDepPo> findCapacityCostDepSelfList(String appName) {
		...
		try {
			this.query(sql, new Object[]{ date, appName, "Ö±½Ó"  }, new SqlCallBack(){
				@Override
				public void readerRows(ResultSet rs) throws Exception {
					CapacityCostDepPo po = new CapacityCostDepPo();
					po.setAppName(rs.getString("app_name"));
