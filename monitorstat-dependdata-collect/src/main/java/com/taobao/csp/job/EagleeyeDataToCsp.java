 ...
 * 
 * @author zhongting.zy
 * 定时把EagleEye的依赖数据分析后导入到CSP的依赖数据库中
 */
public class EagleeyeDataToCsp implements Job {
	
	private static final Logger logger = Logger.getLogger(EagleeyeDataToCsp.class);
   ...
   */
    ...
    logger.info("开始处理EagleEye的数据，构建Key的依赖关系");
		...
		while(true) {
			if(i*EagleeyeDataDao.LIMIT_SIZE < total ||
					(i*EagleeyeDataDao.LIMIT_SIZE >= total && i*EagleeyeDataDao.LIMIT_SIZE - total < EagleeyeDataDao.LIMIT_SIZE)) {
				List<EagleeyeTopoTypeDO> list = eagleDao.getAllTopoStringList(i*EagleeyeDataDao.LIMIT_SIZE);
	...
	/**
	 * 解析拓扑数据并插入到数据库
	 * @param topoStr eagleeye数据库中存储的一个Trace的拓扑数据，深度优先树形结构。
	 * 例如： 
  ...
  public void bulidAppDepRelation() {
    ...
    try {
      ...
      for (CspAppDepAppPo po : list) {
        ...
        if (po.getDepOpsName().equals(po.getOpsName())) {
          continue;
        }
